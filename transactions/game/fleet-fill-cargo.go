...
type FeelFleetCargoTransactionData struct {
...
type FeelFleetCargoTransaction struct {
	Type      t.TransactionType             `cbor:"1,keyasint"`
	From      string                        `cbor:"2,keyasint"`
	Nonce     uint64                        `cbor:"3,keyasint"`
	Data      FeelFleetCargoTransactionData `cbor:"4,keyasint"`
	Signature []byte                        `cbor:"5,keyasint"`
}

func (tx *FeelFleetCargoTransaction) Transaction() (t.Transaction, error) {
...
func (tx *FeelFleetCargoTransaction) Check(ctx t.TransactionContext) *abcitypes.CheckTxResponse {
	...
	if availableCargo < tx.Data.Cargo.Total()+fleet.Cargo.Total() {
		return &abcitypes.CheckTxResponse{
			Code: types.InvalidTransactionError,
		}
	}
...
func (tx *FeelFleetCargoTransaction) Execute(ctx t.TransactionContext) *abcitypes.ExecTxResult {
	result := tx.Check(ctx)
...
func ParseFeelFleetCargoTransaction(tx t.Transaction) (FeelFleetCargoTransaction, error) {
	var txData FeelFleetCargoTransactionData
	err := cbor.Unmarshal(tx.Data, &txData)

	if err != nil {
		return FeelFleetCargoTransaction{}, err
	}

	return FeelFleetCargoTransaction{
		Type:      tx.Type,
		From:      tx.From,
		Nonce:     tx.Nonce,
		Data:      txData,
