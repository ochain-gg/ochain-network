...
type OChainPortalCreditDepositTransactionData struct {
	Type      OChainPortalInteractionType `cbor:"1,keyasint"`
	Arguments []byte                      `cbor:"2,keyasint"`
}

type OChainPortalCreditDepositTransaction struct {
	Type TransactionType
	Data OChainPortalCreditDepositTransactionData
}

func (tx OChainPortalInteractionTransaction) Check(ctx TransactionContext) *abcitypes.ResponseCheckTx {
	switch tx.Data.Type {
...
func (tx OChainPortalInteractionTransaction) Execute(ctx TransactionContext) *abcitypes.ExecTxResult {
	switch tx.Data.Type {
	case NewValidatorPortalInteractionType:
		newValidatorTx, err := ParseNewValidatorTransaction(tx)
		if err != nil {
